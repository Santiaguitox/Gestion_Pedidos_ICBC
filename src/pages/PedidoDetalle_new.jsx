...
import { ArrowLeft, ExternalLink, Plus, Trash2, CheckSquare, Square, Edit, ChevronDown, Copy, Check } from 'lucide-react'
...
function EstadoPopover({ pedido, id, role, onUpdate }) {
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <button
        ...
        style={{
          ...
          opacity: role === ROLES.VIEWER ? 0.5 : 1,
          cursor: role === ROLES.VIEWER ? 'default' : 'pointer',
        }}
      >
      ...
      {open && (
        <div style={{
          ...
          padding: '0.625rem', minWidth: '220px',
          display: 'flex', flexDirection: 'column', gap: '0.25rem',
        }}>
          <p style={{ fontSize: '0.6875rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--text-muted)', padding: '0.25rem 0.5rem 0.5rem' }}>
            Togglear estados
          </p>
          {ESTADOS.map(e => {
            ...
            return (
              <button key={e.value} onClick={() => toggle(e.value)}
                style={{
                  ...
                  background: active ? `${e.color}12` : 'transparent',
...
export default function PedidoDetalle() {
  ...
  async function fetchPedido() {
    ...
    if (data?.entregable?.nombre_pieza !== undefined) {
      setEntregable({
        nombre_pieza: data.entregable.nombre_pieza ?? '',
        link_online: data.entregable.link_online ?? '',
      })
    }
    setLoading(false)
  }
  ...
  async function guardarEntregable() {
    ...
    if (existing) {
      await supabase.from('entregable').update(entregable).eq('id', existing)
    } else {
      await supabase.from('entregable').insert({ ...entregable, pedido_id: id })
    }
    await fetchPedido()
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: '860px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', flexWrap: 'wrap' }}>
          ...
          <EstadoPopover pedido={pedido} id={id} role={role} onUpdate={fetchPedido} />
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {(pedido.subtareas ?? []).map(s => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', padding: '0.5rem 0.625rem', borderRadius: 'var(--radius-sm)' }}>
              ...
              {role === ROLES.ADMIN && (
                <button onClick={() => eliminarSubtarea(s.id)} style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center' }}><Trash2 size={14} /></button>
              )}
            </div>
          ))}
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
            ...
            <input value={entregable.nombre_pieza} onChange={e => setEntregable(f => ({ ...f, nombre_pieza: e.target.value }))} placeholder="Ej: Campaña Día del Padre - Signature" disabled={role === ROLES.VIEWER} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
            <label style={{ fontSize: '0.8125rem', fontWeight: 500, color: 'var(--text-secondary)' }}>Link versión online</label>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <input value={entregable.link_online} onChange={e => setEntregable(f => ({ ...f, link_online: e.target.value }))} placeholder="https://…" disabled={role === ROLES.VIEWER} />
