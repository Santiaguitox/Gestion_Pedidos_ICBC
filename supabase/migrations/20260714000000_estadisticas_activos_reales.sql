-- ============================================================================
-- ESTADÍSTICAS — ajuste v3 (2026-07-12), reemplaza a estadisticas_periodo()
-- de la migración 20260713000000.
--
-- Dos cambios, los dos por un bug real encontrado revisando el código
-- (no un pedido nuevo, sino una corrección):
--
--   1. "activos_hoy" IGNORABA por completo el rango pedido: siempre
--      contaba pedidos activos EN ESTE INSTANTE, sin importar qué
--      p_desde/p_hasta se mandara. Por eso comparar este KPI contra el
--      período anterior no decía nada — las dos consultas devolvían el
--      mismo número. Se reemplaza por una reconstrucción histórica real:
--      "cuántos pedidos estaban activos al cierre de v_hasta", mirando
--      el último cambio_estado de cada pedido hasta esa fecha (no el
--      estado actual). Con el rango "actual" (hasta = hoy) el resultado
--      es idéntico a antes; con un rango pasado, ahora sí refleja el
--      backlog real de ese momento — lo que permite comparar de verdad
--      si el backlog viene creciendo o bajando.
--
--   2. Techo defensivo de ~400 días en el ANCHO del rango, sin importar
--      qué mande el cliente. Hoy la pantalla ya no ofrece rangos libres
--      (se sacaron los presets de días/trimestre/personalizado a favor
--      de meses calendario), así que en el uso normal esto nunca se
--      activa — pero el RPC sigue siendo invocable directo con
--      cualquier p_desde/p_hasta, y a medida que la tabla actividad
--      crezca, una consulta de años enteros sí puede volverse pesada.
--      Es un techo silencioso, no le agrega ningún aviso a la UI.
-- ============================================================================
...
declare
  ...
  v_gran  text;
  v_resultado jsonb;
  -- Techo defensivo — ver punto 2 del header. No es un valor que la UI
  -- exponga ni permita elegir, es solo protección de motor.
